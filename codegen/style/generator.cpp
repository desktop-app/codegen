...
QString Generator::valueAssignmentCode(
	...
	case Tag::Icon: {
		...
		const auto keyArg = useContextPx
			? QStringLiteral("context.key")
			: QStringLiteral(
				"style::MakeScaleKey(scale, style::DevicePixelRatio())");
		for (const auto &part : v.parts) {
			...
			parts.push_back(QString(
				"style::internal::MonoIcon{ &iconMask%1, %2, %3, %4 }"
			).arg(
...
bool Generator::writeMasksSource() {
	...
	for (auto i = iconMasks_.cbegin(), e = iconMasks_.cend(); i != e; ++i) {
		...
		} else if (QFileInfo(fileInfo.dir().absoluteFilePath(
				fileInfo.fileName().split('-')[0] + ".svg")).exists()) {
